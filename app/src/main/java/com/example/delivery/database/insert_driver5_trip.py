...
def main():
    ...
    print("\n--- ⚙️ CLEANING UP OLD TEST TOUR DATA ---")
    driver_id = 5
    clean_queries = [
        f'DELETE FROM "TripShipmentLink" WHERE "tripId" IN (SELECT id FROM "Trip" WHERE "driverId" = {driver_id} AND "tripDate"::text LIKE \'2026-05-19%\')',
        f'DELETE FROM "TripStop" WHERE "tripId" IN (SELECT id FROM "Trip" WHERE "driverId" = {driver_id} AND "tripDate"::text LIKE \'2026-05-19%\')',
        f'DELETE FROM "ShipmentProof" WHERE "shipmentId" IN (SELECT id FROM "Shipment" WHERE "driverId" = {driver_id} AND "plannedStart"::text LIKE \'2026-05-19%\')',
        f'DELETE FROM "ShipmentLine" WHERE "shipmentId" IN (SELECT id FROM "Shipment" WHERE "driverId" = {driver_id} AND "plannedStart"::text LIKE \'2026-05-19%\')',
        f'DELETE FROM "Shipment" WHERE "driverId" = {driver_id} AND "plannedStart"::text LIKE \'2026-05-19%\'',
        f'DELETE FROM "Trip" WHERE "driverId" = {driver_id} AND "tripDate"::text LIKE \'2026-05-19%\''
    ]
    ...
    shipment_details = [
        ("Colis d''équipements électroniques", 10, "PCS", 15.0, 0.1, "09:00:00", "10:00:00", 465.0, "1 Place de la République", "Strasbourg", "67000"),
        ("Palette de produits manufacturés", 2, "PAL", 250.0, 1.5, "10:30:00", "11:30:00", 150.0, "5 Quai de la Loire", "Nantes", "44000"),
        ("Cartons d''archives confidentielles", 15, "CTN", 45.0, 0.4, "12:00:00", "13:00:00", 310.0, "Avenue de la Gare 25", "Lausanne", "1003"),
        ("Lot de stéthoscopes et matériel médical", 5, "BOX", 8.0, 0.05, "14:00:00", "15:00:00", 230.0, "100 Route de Narbonne", "Toulouse", "31000"),
        ("Fournitures de bureau standards", 30, "PCS", 12.0, 0.08, "15:30:00", "16:30:00", 380.0, "15 Promenade des Anglais", "Nice", "06000")
    ]
    ...
    print("\n✅ SUCCESS: Tournée de test créée de A à Z avec succès sur Neon PostgreSQL pour Pierre Bernard (Driver ID: 5)!")
