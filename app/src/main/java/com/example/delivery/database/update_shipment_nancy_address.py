...
def main():
    print("--- ⚙️ UPDATING NANCY SHIPMENT ADDRESS FOR BETTER GEOCODING ---")
    
    shipment_id = 160
    
    # Update with a more well-known address in Nancy
    update_shipment_query = f"""
        UPDATE "Shipment" 
        SET "deliveryAddress" = 'Place Stanislas', 
            "deliveryCity" = 'Nancy', 
    ...
    if res_shipment:
        print(f"✅ Shipment {shipment_id} address updated to Place Stanislas, Nancy 54000")
    else:
