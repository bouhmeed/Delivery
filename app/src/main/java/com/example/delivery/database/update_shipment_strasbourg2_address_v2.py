...
def main():
    print("--- ⚙️ UPDATING STRASBOURG 2 SHIPMENT ADDRESS TO AVOID PARIS CONFUSION ---")
    
    shipment_id = 159
    
    # Update with explicit Strasbourg cathedral name
    update_shipment_query = f"""
        UPDATE "Shipment" 
        SET "deliveryAddress" = 'Cathédrale Notre-Dame de Strasbourg', 
    ...
    if res_shipment:
        print(f"✅ Shipment {shipment_id} address updated to Cathédrale Notre-Dame de Strasbourg, Strasbourg 67000")
    else:
        print(f"❌ Failed to update shipment {shipment_id} address")
