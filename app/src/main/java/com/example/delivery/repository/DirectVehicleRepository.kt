        ...
        return try {
            ...
            if (rows != null && rows.length() > 0) {
                ...
                val vehicle = Vehicle(
                    ...
                    dernierControle = vehicleJson.optString("dernierControle", null),
                    driverId = if (vehicleJson.has("driverId") && !vehicleJson.isNull("driverId")) {
                        vehicleJson.getInt("driverId").toString()
                    } else {
                        null
                    },
                    prochainControle = vehicleJson.optString("prochainControle", null),
                    status = vehicleJson.optString("status", "ACTIVE"),
                    type = vehicleJson.optString("type", "CAMION"),
                    createdAt = vehicleJson.optString("createdAt", null),
                    updatedAt = vehicleJson.optString("updatedAt", null)
                )
            ...
            if (rows != null && rows.length() > 0) {
                ...
                for (i in 0 until rows.length()) {
                    ...
                    val maintenance = VehicleMaintenance(
                        ...
                        nextMaintenance = maintenanceJson.optString("nextMaintenance", null),
                        ...
                        notes = maintenanceJson.optString("notes", null),
                        technician = maintenanceJson.optString("technician", null),
                        status = maintenanceJson.optString("status", "PENDING"),
                        vehicleName = maintenanceJson.optString("vehicleName", null),
                        registration = maintenanceJson.optString("registration", null)
                    )
