...
import com.example.delivery.network.api.delivery.ShipmentDatesResponse
...
class DirectShipmentRepository {
        ...
        try {
            ...
            if (linkResult != null && linkResult.length() > 0) {
                ...
                val response = StatusUpdateResponseV2(
                    ...
                    data = com.example.delivery.network.api.delivery.StatusUpdateDataV2(
                        tripShipmentLink = com.example.delivery.network.api.delivery.TripShipmentLinkUpdate(id = tripShipmentLinkId, status = status, podDone = false, updatedAt = ""),
                        shipment = null,
    ...
    suspend fun searchShipment(barcode: String, driverId: Int): Result<com.example.delivery.models.delivery.ShipmentSearchResponse> = performSearch(barcode, driverId)

    suspend fun searchByTrackingNumber(trackingNumber: String, driverId: Int): Result<com.example.delivery.models.delivery.ShipmentSearchResponse> = performSearch(trackingNumber, driverId)

    suspend fun markAsDelivered(shipmentId: Int, driverId: Int): Result<com.example.delivery.models.delivery.ShipmentSearchResponse> = withContext(Dispatchers.IO) {
        try {
    ...
    private suspend fun performSearch(queryInput: String, driverId: Int): Result<com.example.delivery.models.delivery.ShipmentSearchResponse> = withContext(Dispatchers.IO) {
        try {
            ...
            if (rows == null || rows.length() == 0) {
                return@withContext Result.success(com.example.delivery.models.delivery.ShipmentSearchResponse(
                    success = false,
                    data = null,
            ...
            val shipmentDetail = com.example.delivery.models.delivery.ShipmentSearchDetail(
                id = shipmentId,
                shipmentNo = row.optString("shipmentNo", ""),
                trackingNumber = if (row.isNull("trackingNumber")) null else row.optString("trackingNumber", null as String?),
                status = row.optString("status", "TO_PLAN"),
                description = row.optString("description", ""),
                quantity = row.optInt("quantity", 1),
                deliveryAddress = if (row.isNull("dest_addr")) null else row.optString("dest_addr", null as String?),
                deliveryCity = if (row.isNull("dest_city")) null else row.optString("dest_city", null as String?),
                deliveryZipCode = if (row.isNull("dest_zip")) null else row.optString("dest_zip", null as String?),
                customerId = customerId,
                priority = row.optString("priority", "MEDIUM"),
                plannedStart = if (row.isNull("plannedStart")) null else row.optString("plannedStart", null as String?),
                plannedEnd = if (row.isNull("plannedEnd")) null else row.optString("plannedEnd", null as String?)
            )
            
            val clientInfo = com.example.delivery.models.delivery.ClientInfo(
                id = customerId,
                ...
                phone = if (row.isNull("client_phone")) null else row.optString("client_phone", null as String?)
            )
            ...
            val searchData = com.example.delivery.models.delivery.ShipmentSearchData(
            ...
            Result.success(com.example.delivery.models.delivery.ShipmentSearchResponse(
                success = true,
                data = searchData,
