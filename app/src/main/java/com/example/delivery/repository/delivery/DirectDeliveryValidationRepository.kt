        ...
        try {
            ...
            if (hasProof) {
                ...
                val updateProofQuery = """
                    UPDATE "ShipmentProof"
                    SET "imageUrl" = '$escapedImage', "createdAt" = CURRENT_TIMESTAMP
                    WHERE id = $proofId
                """.trimIndent()
                DatabaseManager.executeQuery(updateProofQuery)
            } else {
                ...
                val insertProofQuery = """
                    INSERT INTO "ShipmentProof" ("shipmentId", "imageUrl", "signatureUrl", "createdAt")
                    VALUES ($shipmentId, '$escapedImage', '', CURRENT_TIMESTAMP)
                """.trimIndent()
                DatabaseManager.executeQuery(insertProofQuery)
            }

            // 2. Update Shipment description to include returns details
            val updateShipmentQuery = """
                UPDATE "Shipment"
                SET "description" = CONCAT(COALESCE("description", ''), '\n[Retours: Palettes: $palettes, Caisses: $caisses, Bouteilles: $bouteilles, Fûts: $futs, Autre: $autre. Emballages: $packagingRecovered, Colis: $packagesRecovered. Commentaire: $escapedComment]'), "updatedAt" = CURRENT_TIMESTAMP
                WHERE id = $shipmentId
            """.trimIndent()
            DatabaseManager.executeQuery(updateShipmentQuery)

            // 3. Update TripShipmentLink to set returnsDone = true
            val updateTripLinkQuery = """
                UPDATE "TripShipmentLink"
